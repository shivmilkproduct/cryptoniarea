...
app.post("/submit-case-request", async (req, res) => {
    try {
        const { case: caseId, name, email, mobile, whatsapp, occupation, level, submittedAt } = req.body || {};
        ...
        const message =
            ...
            `Email: ${safe(email)}\n` +
            `Mobile: ${safe(mobile)}\n` +
            `WhatsApp: ${safe(whatsapp || "Same as mobile")}\n` +
            `Occupation: ${safe(occupation || "Not specified")}\n` +
            `Skill Level: ${safe(level || "Not specified")}\n` +
